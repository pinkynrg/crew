...
- `src/*.js` — the CLI source, ESM modules: `main` (entry: parseArgs/dispatch), `commands`
  (all `cmd*`, help, guards, overrides, check, wizard), `runner` (`runFanout`, guards, wiring
  materialization), `wiring` (URL match, `wireText`, overrides, graph/connectivity, `resolveRun`),
  `config` (load/merge/migrate, `local.json`, key-sets), `selection` (`selectMembers`), `prompt`
  (`menu`/`makePrompter`), `colors`, `util`, `pkg`. No import cycles (`config` never imports
  `wiring`/`prompt`; that's why `selectMembers` lives in `selection`).
- `bin/crew.js` — the **esbuild bundle** of `src/main.js` (single file, `#!/usr/bin/env node`
  via `--banner`). Build artifact: gitignored/untracked, regenerated by `npm run build`
  (`prepare` runs it on `npm ci`/`npm publish`). This is what ships + what `bin.crew` points at.
- `package.json` — `bin.crew`, `type:module`, `engines.node >=18`; `build`/`prepare` scripts;
  `esbuild` is the ONLY devDep (pinned via lockfile). `files: ["bin/crew.js","README.md"]`;
  `.npmignore` keeps the built bin in the tarball despite `.gitignore`.
- `.github/workflows/publish.yml` — npm publish CI (push to main; auto-bump patch; OIDC
  trusted publishing, npm@11, `--provenance`). `npm ci` runs `prepare` so the bundle exists.
...
- **Zero *runtime* dependencies.** Node built-ins only (`node:fs`, `node:path`, `node:os`,
  ...
  runner is our own (`runFanout`). `esbuild` is a build-time devDep only — nothing third-party
  runs at crew runtime.
- **One shipped file, modular source.** Source is split across `src/*.js`; esbuild bundles it
  into a single `bin/crew.js`. Add new code as a module under `src/`; never hand-edit the
  generated `bin/crew.js`. Run `npm run build` after changing `src/` to refresh the bundle.
...
No test framework. Run the source directly (unbundled ESM works) or build first:

```sh
node src/main.js --config /tmp/x.json list             # run source directly (no build)
npm run build && node --check bin/crew.js              # or refresh + syntax-check the bundle
node src/main.js --config /tmp/x.json graph            # read-only, no TTY needed
node src/main.js --config /tmp/x.json check            # validate; exit 1 on errors
```
...
`crew check` (`cmdCheck`) is the hand-rolled, zero-runtime-dep config validator — NO
JSON-Schema library (would be a runtime dep) and NO separate schema file (cross-refs below
can't be expressed in one anyway). It validates the merged config + `local.json`: known-key sets
